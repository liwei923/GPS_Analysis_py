...
####切勿外传####
###################################################################
# 版本要求：windows ; arcgis10.8 ; python2.7 ;
# arcgis10.8安装教程:https://h5.qzone.qq.com/ugc/share?sharetag=7CD65FCC12F8D3C307D8D921E5EF95D7&bp7=&bp2=&bp1=&_wv=1&res_uin=2228246361&appid=2&cellid=1612746639&no_topbar=1&subid=&g_ut=3&from=mp
###################################################################
### 你只需要修改这个部分里的内容
fp1 = "F:/data" # 新建一个工作文件夹 # 将在googleearth里绘制的kml断层线保存到这个文件夹
fp2 = "F:/data/projections.gdb" # 在工作文件夹下新建一个gdb，在这个gdb里放置数据库格式的GPS,必须命名为inGPS_WGS
faultname = "haiyuanfault.kml"
...
r1 = fp2 + "/Polylines"
r2 = fp2 + "/Polylines_UTM"
r3 = fp2 + "/outGPS_UTM"
r4 = fp2 + "/rectangle01_UTM"
r5 = fp2 + "/mysites"
arcpy.Delete_management(r1)
arcpy.Delete_management(r2)
arcpy.Delete_management(r3)
arcpy.Delete_management(r4)
arcpy.Delete_management(r5)
###################################################################
...
###################################################################
# 建立缓冲区形成剖面矩形
arcpy.Buffer_analysis("Polylines_UTM", "rectangle01_UTM", profile_len, "FULL", "FLAT", "LIST") 
# 提示成功
print("step03_buffer ok!")
###################################################################
# 按照剖面矩形提取GPS站点
arcpy.Intersect_analysis (["outGPS_UTM", "rectangle01_UTM"], "mysites", "ALL", "", "")
# 提示成功
